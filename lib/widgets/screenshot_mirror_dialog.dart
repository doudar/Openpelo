...
class ScreenshotMirrorDialog extends StatefulWidget {
  final String? reason;
  final VoidCallback? onTryStreaming;
  const ScreenshotMirrorDialog({super.key, this.reason, this.onTryStreaming});

  @override
  State<ScreenshotMirrorDialog> createState() => _ScreenshotMirrorDialogState();
}

class _ScreenshotMirrorDialogState extends State<ScreenshotMirrorDialog> {
  ...
  void initState() {
    super.initState();
    final device = context.read<AppProvider>().selectedDevice;
    _serial = device?.serial;
    _identity = device?.identityKey;
    _startLoop();
  ...
  bool _sameDevice(AppProvider provider) =>
      _serial != null &&
      provider.selectedDevice?.serial == _serial &&
      provider.selectedDevice?.identityKey == _identity;
  ...
  Future<void> _tapPreview(
    ...
    final provider = Provider.of<AppProvider>(context, listen: false);
    if (!_sameDevice(provider)) return;
  ...
  Future<void> _press(Future<bool> Function() action) async {
    if (!_sameDevice(context.read<AppProvider>())) return;
    await action();
  }
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            if (widget.onTryStreaming != null)
              TextButton(
                onPressed: widget.onTryStreaming,
                child: const Text('Try fast streaming'),
              ),
